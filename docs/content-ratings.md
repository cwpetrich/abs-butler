...
| **Apple Books** (iTunes Search API) | **Implemented as a metadata source, and deliberately not as a rating one.** Free, keyless, and the only source here covering ebooks as well as audiobooks. Its categories looked ideal — "Dystopian Fiction for Young Adults", "Counting & Numbers for Kids" — and measured *worse* than having no Apple at all: 3/8 against Open Library's 4/8, moving The Very Hungry Caterpillar and Goodnight Moon to middle-grade. Apple files everything from board books to age twelve under "Kids", so its labels cannot separate picture-book from middle-grade. Dropping the "Kids" family and keeping the precise "for Young Adults" one was measured too: 3/8 again, this time pushing Charlotte's Web to young-adult. Making these usable needs rules written for Apple's vocabulary and re-measured against the table above. |
