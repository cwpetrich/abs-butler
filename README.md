...
`--update` updates the script itself first and re-runs with it, so it stays current on its own. It
refreshes the compose file, pulls the current image, restarts, and changes no settings of its
